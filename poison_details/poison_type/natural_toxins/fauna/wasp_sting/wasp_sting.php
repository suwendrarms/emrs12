...
<section id="waspsting" class="page-section color">
  <div class="container">
                ...
                <div class="padd">
                  ...
                  <div class="form quick-post">
                    ...
                    <form class="form-horizontal" method="post" action=""></form>-->
					  ...
					  <div class="pull-right">
                          <a href="#waspsting" class="btn btn-danger scroll-link" >Back</a>
...
<?php
	...
	/* $querry1 = $connect->query("SELECT * FROM details_about_wasp_stings"); // or die('querry 1 is not work');
	
	while($row = $querry1->fetch_assoc())
	{
		echo " ".$row["Index_Of_wasp_Stings"]."".$row["Place_Of_Stings"]."<br>";
	} */
	...
	if(isset($_POST['submit'])!="")
	{
		...
		$index_of_details_about_wasp_stings = 'fn-ws'.time().'-'.$id;
		...
		$querry1 = $connection->query("INSERT INTO details_about_wasp_stings (index_of_details_about_wasp_stings, Patient_Id, Time_Of_Stings, Number_of_stings, Place_Of_Stings, Head_and_Neck , Upper_Limb, Chest, Abdomen, Lower_Limb, other, Circumstances_Of_Stings) VALUES ('$index_of_details_about_wasp_stings', '$id', '$timeofsting', '$numberofsting', '$placeofsting', '$Head_and_Neck','$Upper_Limb', '$Chest', '$Abdomen', '$Lower_Limb', '$other', '$cos')") or die(mysqli_error($connection));
	...
	$Index_Of_clinical_features_wasp_stings = time().'-'.$id;
	...
	$Rhabdomyolysis  =  $_POST['Rhabdomyolysis'];
		...
		$querry2 = $connection->query("INSERT INTO clinical_features_wasp_stings (Index_Of_clinical_features_wasp_stings, Patient_Id, Burning_pain, Swelling, Pruntus, Nauka, Vomiting, Hypotenisum, Bronchospasm, Oliguna, Renal_failure, Diarrhoea, Tightness_of_chest, Malaise, Urticana, Facial_odema, Laryngeal_odema, Seizure, Rhabdomyolysis, Features_of_anaphylaxis) VALUES ('$Index_Of_clinical_features_wasp_stings', '$id', '$Burning_pain' ,'$Swelling' ,'$Pruntus' ,'$Nauka' ,'$Vomiting' ,'$Hypotenisum' ,'$Bronchospasm' ,'$Oliguna' ,'$Renal_failure' ,'$Diarrhoea' ,'$Tightness_of_chest' ,'$Malaise' ,'$Urticana' ,'$Facial_odema' ,'$Laryngeal_odema' ,'$Seizure' ,'$Rhabdomyolysis', '$foa')") or die(mysqli_error($connection));
	...
	$index_of_management_of_wasp = 'mow-'.time().'-'.$id;
		
		
		$querry3 = $connection->query("INSERT INTO management_of_wasp (Index_Of_management_Of_Wasp, Patient_Id, Apply_ice_packs, Artihistamine, Adranaline_IM, ICU_care, Steroids, Need_Renal_Replacement_therapy, Need_invasive_ventilation, Stinger_scrapped) VALUES ('$index_of_management_of_wasp', '$id',  '$applyicepacks', '$artihistamine','$adranaline', '$icucare', '$steroids', '$needrenalrep', '$needinvasive', '$stingerscrapped') ") or die('querry 3 does not work');
	}
